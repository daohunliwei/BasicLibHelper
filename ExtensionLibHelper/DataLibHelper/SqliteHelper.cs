...
namespace ExtensionLibHelper.DataLibHelper
{
    ...
    public static class SQLiteHelper
    {
        ...
        public static DataRow ExecuteDataRow(string connectionString, CommandType commandType, string commandText, params SQLiteParameter[] commandParameters)
        {
        ...
        {
            //DateTime timeStart = DateTime.Now;
            DataSet ds = ExecuteDataSet(connectionString, commandText, commandType);
            //if (DateTime.Now.Subtract(timeStart).TotalMilliseconds > 600)//测试数据库读取时间600ms
            //{
        ...
        {
            //DateTime timeStart = DateTime.Now;
            DataSet ds = ExecuteDataSet(connectionString, commandText, commandType, commandParameters);
            //if (DateTime.Now.Subtract(timeStart).TotalMilliseconds > 600)//测试数据库读取时间600ms
            //{
            //    LogHelper.LogHelper.WriteLog("WRN", "DBO", "SQLHelper", "ExecuteDataTable", "数据库查询时间超过600ms。耗时：" + DateTime.Now.Subtract(timeStart).TotalMilliseconds + "ms", null);
            //}
